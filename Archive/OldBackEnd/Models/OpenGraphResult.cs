...
namespace BackEnd.Models
{
    public class OpenGraphResult
    {
        ...
        public OpenGraphResult(string id, OpenGraph graph, params HtmlNode[] nodes)
        {
            ...
            Title = string.IsNullOrEmpty(graph.Title) ? 
                title : HtmlEntity.DeEntitize(graph.Title);
            ...
            var descriptionNode = nodes.FirstOrDefault(n => 
                n.Attributes.Contains("content")
                && n.Attributes.Contains("name")
                && n.Attributes["name"].Value == "description");

            Description = HtmlEntity.DeEntitize(
                graph.Metadata["og:description"].FirstOrDefault()?.Value) 
                ?? descriptionNode?.Attributes["content"].Value;
        }
